...
CASES = (
    ...
    ("request-recording-outage", "query_logs", SCOPE),
)
        ...
        try:
            ...
            for case_id, name, arguments in CASES:
                ...
                with ExitStack() as stack:
                    ...
                    if case_id in {"recording-outage", "request-recording-outage"}:
                        original_record = RecordingTools._record
                        failing_kind = (
                            "tool_call" if case_id == "recording-outage" else "tool_request"
                        )

                        async def unavailable(
                            self, kind, payload, _original=original_record, _failing=failing_kind
                        ):
                            if kind == _failing:
                                raise OSError("synthetic event storage failure")
                            return await _original(self, kind, payload)
...
def main():
    ...
    for case in result["cases"]:
        print(
            ...
            "unresolved:",
            len(case["review"]["unresolved_requests"]),
        )
