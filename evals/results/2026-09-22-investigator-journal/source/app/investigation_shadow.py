...
def review_events(scope, events):
    ...
    for event in events:
        ...
        if event.get("kind") == "tool_request":
            ...
            if (
                ...
                continue
            requests[sequence] = pending[sequence] = (name, arguments)
            row.update(tool=name, observed="requested", baseline=baseline(scope, name, arguments))
            continue
        ...
        if "request_sequence" in payload:
            ...
            if (
                ...
                or pending[request_sequence] != (name, payload.get("arguments"))
            ):
                continue
        ...
        expected = baseline(scope, name, payload.get("arguments"))
    ...
    return {
        ...
        "disagreements": sum(not row["agrees"] for row in calls),
