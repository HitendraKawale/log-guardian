...
C uses `--system C` through the same CLI and `run_investigation` interface. Before its first model request, the server reads an unfiltered sample within the owner scope, returning at most 50 rows and 16 KiB. The trace and worker journal mark this read `origin=server_initial`; it is not a model-selected action. The model can then select the four read-only tools or return a report. The initial read consumes one tool execution and shares the existing evidence/deadline budgets. It is not exhaustive: truncation and source errors remain explicit. Native tool calls execute sequentially with validated arguments and scope checks. Limits are six model requests, eight tool executions, 120 seconds, 64 KiB total evidence, and 1,024 output tokens per request. Duplicate normalized queries, unknown tools and malformed arguments stop the run. There are no automatic retries, staying below the plan's one-retry ceiling and avoiding ambiguous billing.

C preserves partial evidence and per-request usage in `model_calls`. Unknown request usage makes the run's total usage/cost null and stops execution. Missing cache counts receive no discount; aggregate cached tokens sum only known counts, with `cache_usage_complete=false` when any request omitted them. Cost reservations include the growing message history and prior known spend. Its dry run collects the same bounded initial evidence and previews the first request's reservation without calling a provider; it does not predict subsequent queries or total cost. Runs without a configured provider fail before this initial read unless explicitly dry-running.

Offline scripted SDK tests demonstrate initial evidence delivery, evidence-dependent next queries and boundary handling, not live model quality. This initial-read candidate has not been evaluated against the provider. Existing paid batches are closed and cannot authorize a new run.
